...
export default function CVEOpsPage({ API, apiFetch, hasRole, getToken, AppIcon }) {
  ...
  const linkedRecords = cves.filter(cve => Number(cve.affected_hosts || 0) > 0).length;
  ...
  const summaryCards = [
    { label: 'Tracked CVEs', value: totalTracked, sub: `${cves.length} in the current view`, icon: 'database', color: '#2563eb', bg: 'rgba(37,99,235,0.12)' },
