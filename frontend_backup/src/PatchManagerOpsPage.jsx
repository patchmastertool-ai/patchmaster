...
export default function PatchManagerOpsPage({ hosts, API, apiFetch, AppIcon, useInterval }) {
  ...
  const fetchUpgradable = async () => {
    ...
    try {
      ...
      setInstPage(0);
      const hostLabel = hosts.find(h => h.ip === selectedHost)?.hostname || selectedHost;
  ...
  const pushRepoPackageToHost = async () => {
    ...
    const hostObj = hosts.find(h => h.ip === selectedHost);
  ...
  const currentHost = hosts.find(host => host.ip === selectedHost) || null;
  ...
  return (
    <div className="ops-shell">
      ...
      {selectedHost && upgradable.length === 0 && !loading && !info && (
        <div className="ops-empty">Run an update check to inventory pending packages for {hosts.find(h => h.ip === selectedHost)?.hostname || selectedHost}.</div>
      )}
