...
export default function HostsOpsPage({ hosts, setHosts, API, apiFetch, hasRole, AppIcon, useInterval }) {
  ...
  }, hosts.length ? 15000 : null);

  const deleteHost = (id) => {
    if (!window.confirm('Delete this host?')) return;
    apiFetch(`${API}/api/hosts/${id}`, { method: 'DELETE' }).then(refreshHosts);
  };
  ...
  const osFamily = (host) => {
    const groups = (host.groups || []).map(group => (group || '').toLowerCase());
  ...
  }, [hosts, search, osFilter, siteFilter]);
  ...
  const onlineCount = hosts.filter(isHostOnline).length;
  const avgCompliance = totalHosts ? Math.round(hosts.reduce((sum, host) => sum + Number(host.compliance_score || 0), 0) / totalHosts) : 0;
  const rebootCount = hosts.filter(needsReboot).length;
  const totalCves = hosts.reduce((sum, host) => sum + Number(host.cve_count || 0), 0);
  const highRiskHosts = hosts.filter(host => Number(host.cve_count || 0) > 0 || Number(host.compliance_score || 0) < 80).length;
  const osCounts = hosts.reduce((acc, host) => {
    acc[osFamily(host)] = (acc[osFamily(host)] || 0) + 1;
    return acc;
  }, { windows: 0, debian: 0, rhel: 0, arch: 0, opensuse: 0, alpine: 0, freebsd: 0, other: 0 });
  const siteOptions = useMemo(() => {
    const names = Array.from(new Set(hosts.map(host => (host.site || '').trim()).filter(Boolean)));
