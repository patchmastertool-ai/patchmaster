...
from api.network_boot import public_router as network_boot_public_router, router as network_boot_router
...
async def lifespan(app: FastAPI):
    ...
    auto_create = os.getenv("PM_AUTO_CREATE_TABLES", "1").strip().lower() in {"1", "true", "yes", "y", "on"}
        ...
        try:
            ...
            try:
                ...
                async with _async_session() as _db:
                    ...
                    if migrated:
                        logger.info("Migrated %s legacy inline CI/CD or Git secrets to encrypted storage.", migrated)
                    await _db.commit()
...
def _request_trace_from_state(request: Request) -> tuple[str, str]:
    ...
    trace_token = str(getattr(request.state, "trace_token", "") or "").strip() or uuid.uuid4().hex
    return request_id, trace_token


def _error_envelope(request: Request, status_code: int, detail: object, error_type: str) -> dict:
...
class TraceMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        trace_token = str(request.headers.get("x-trace-token", "")).strip() or uuid.uuid4().hex
...
async def request_validation_envelope_handler(request: Request, exc: RequestValidationError):
...
async def unexpected_exception_envelope_handler(request: Request, exc: Exception):
    expose_reason = os.getenv("PM_EXPOSE_INTERNAL_ERROR_REASON", "0").strip().lower() in {"1", "true", "yes"}
    ...
    async def dispatch(self, request, call_next):
        ...
        if not info.get("valid", False):
            return JSONResponse(
                status_code=403,
                content={
                    "detail": info.get("error", "License is invalid. Please activate a valid license."),
            ...
            if path.startswith(prefix):
                ...
                if "backups" in prefix:
                    if not has_backup_feature:
                        return JSONResponse(status_code=403, content={"detail": "Backups not included in license"})
