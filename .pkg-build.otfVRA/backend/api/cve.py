...
import io
import re
from fastapi import APIRouter, Depends, HTTPException, Query, Header, status, Request, UploadFile, File
import jwt
...
from auth import get_current_user, get_current_user_optional, require_role, SECRET_KEY, ALGORITHM
...
def _extract_nvd_fields(cve_data: dict) -> dict:
    ...
    for key in ("cvssMetricV31", "cvssMetricV30", "cvssMetricV2"):
        ...
        if metric:
            ...
            sev_raw = metric[0].get("baseSeverity") or cvss_data.get("baseSeverity") or ""
...
async def cve_stats(db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    by_severity = {}
    for sev in Severity:
        count = await db.scalar(select(func.count(CVE.id)).where(CVE.severity == sev)) or 0
        by_severity[sev.value] = count

    open_vulns = await db.scalar(
        select(func.count()).select_from(HostCVE).where(HostCVE.status == "active")
    ) or 0
    patched_vulns = await db.scalar(
...
async def create_cve(
    ...
    if not CVE_ID_PATTERN.match(data.cve_id):
        raise HTTPException(400, f"Invalid CVE ID format: '{data.cve_id}'. Expected CVE-YYYY-NNNNN.")
...
async def export_cve_report(
    ...
    # Accept token via Authorization header or auth cookie only — NOT query params
    resolved_token = None
    ...
    if user:
        ...
        raise HTTPException(status.HTTP_401_UNAUTHORIZED, "Not authenticated. Provide Authorization: Bearer <jwt> header.")
    ...
    if authed_user.role not in (UserRole.admin, UserRole.operator, UserRole.auditor):
        raise HTTPException(status.HTTP_403_FORBIDDEN, "Insufficient permissions to export CVE data.")

    result = await db.execute(select(HostCVE).options(selectinload(HostCVE.cve), selectinload(HostCVE.host)))
    mappings = result.scalars().all()

    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(["CVE ID", "Severity", "CVSS", "Host", "IP", "Status", "Detected At", "Description"])
    ...
    return StreamingResponse(
        ...
        media_type="text/csv",
        headers={"Content-Disposition": f"attachment; filename=cve_report_{datetime.now().strftime('%Y%m%d')}.csv"}
    )


@router.get("/{cve_id}")
async def get_cve(cve_id: str, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
...
async def import_cves(
    ...
    def normalize_row(row: dict) -> Optional[dict]:
        ...
        host_tokens = []
        for key in ("host_id", "host_ids", "host", "hosts", "hostname", "ip", "host_ip"):
    ...
    for row in records:
        # Validate CVE ID format before any DB operation
        ...
        if row.get("host_refs"):
            ...
            if labels:
                conds.append(or_(func.lower(Host.hostname).in_([l.lower() for l in labels]), func.lower(Host.ip).in_([l.lower() for l in labels])))
            if conds:
                ...
                for host in hosts_found:
                    ...
                    if not hc:
                        db.add(HostCVE(host_id=host.id, cve_id=row["cve_id"], status="active"))
                        host.cve_count = (host.cve_count or 0) + 1
                        host_links_created += 1
    await db.commit()
    return {"created": created, "updated": updated, "processed": len(records), "host_links": host_links_created}
...
async def map_cve_to_host(
    ...
    host.cve_count = await db.scalar(
        select(func.count()).select_from(HostCVE).where(HostCVE.host_id == host.id, HostCVE.status == "active")
    ) or 0
    await db.commit()
...
async def sync_cves_from_nvd(
    ...
    if source_name == "cis":
        ...
        cis_username = (payload.cis_username if payload else None) or os.environ.get("CIS_USERNAME", "")
        cis_password = (payload.cis_password if payload else None) or os.environ.get("CIS_PASSWORD", "")
        cis_username = cis_username.strip()
        cis_password = cis_password.strip()
        try:
            async with httpx.AsyncClient(timeout=30, follow_redirects=True) as client:
                auth = httpx.BasicAuth(cis_username, cis_password) if cis_username and cis_password else None
        ...
        found_ids = sorted({match.group(0).upper() for match in CVE_ID_PATTERN.finditer(text)})
        if not found_ids:
            lowered = text.lower()
            login_markers = ("sign in", "log in", "login", "password")
            if any(marker in lowered for marker in login_markers) and not (cis_username and cis_password):
                raise HTTPException(
                    status.HTTP_401_UNAUTHORIZED,
                    detail={
        ...
        async with httpx.AsyncClient(timeout=30) as nvd_client:
            for cve_id in found_ids:
                ...
                try:
                    ...
                    if vulnerabilities:
                        nvd_fields = _extract_nvd_fields(vulnerabilities[0].get("cve", {}) or {})
                        enriched += 1
                except Exception:
                    nvd_fields = None
                ...
                if existing:
                    ...
                    existing.affected_products = affected_products or existing.affected_products
                    existing.source = "cis"
                    updated += 1
                else:
                    db.add(
    ...
    if source_name != "nvd":
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "Unsupported source. Use nvd or cis.")
