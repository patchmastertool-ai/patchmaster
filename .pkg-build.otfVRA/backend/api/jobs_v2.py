...
async def list_jobs(
    ...
    if status:
        try:
            ...
            raise HTTPException(400, f"Invalid status '{status}'. Valid values: {[s.value for s in JobStatus]}")
    if host_id:
...
async def update_job(
    ...
    # Bug #3 fix: commit first so status is persisted before side-effect checks
    await db.commit()
    await db.refresh(job)
...
async def get_job_status(
    ...
    if not job:
        raise HTTPException(404, "Job not found")
    # Bug #2 fix: use SELECT ... FOR UPDATE to prevent concurrent reads from
    # each triggering the same status transition (optimistic guard via re-check).
    if job.status == JobStatus.pending:
        age_seconds = 0
        if job.created_at:
            age_seconds = max(int((_utcnow() - job.created_at).total_seconds()), 0)
        if age_seconds >= 120:
            # Re-fetch with a row lock so only one concurrent reader wins
            locked = await db.execute(
                select(PatchJob).where(PatchJob.id == job_id, PatchJob.status == JobStatus.pending).with_for_update(skip_locked=True)
            )
...
async def rollback_job(
    ...
    if job.status not in [JobStatus.success, JobStatus.failed]:
        raise HTTPException(400, f"Cannot rollback a job with status '{job.status}'. Only success/failed jobs can be rolled back.")
    ...
    try:
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            # Bug #12 fix: check agent result for Windows "not supported" error
            # and mark as failed instead of rolled_back
            agent_success = (
                r.status_code == 200
                and result_data.get("success", True)
                and "not supported" not in str(result_data.get("error", "")).lower()
            )
            rollback_job.status = JobStatus.rolled_back if agent_success else JobStatus.failed
    ...
    await db.commit()
    await db.refresh(rollback_job)
    result2 = await db.execute(select(PatchJob).options(selectinload(PatchJob.host)).where(PatchJob.id == rollback_job.id))
        ...
        try:
            while True:
                result = await session.execute(
                    select(PatchJob).options(selectinload(PatchJob.host)).where(PatchJob.id == int(job_id))
                )
                job = result.scalar_one_or_none()
                if not job:
                ...
                # Stop loop if job is in a terminal state
                if job.status in [JobStatus.success, JobStatus.failed, JobStatus.rolled_back, JobStatus.aborted]:
                    break
