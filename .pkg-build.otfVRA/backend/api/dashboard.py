...
async def dashboard_summary(db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    recent_result = await db.execute(select(PatchJob).order_by(desc(PatchJob.created_at)).limit(5))
    recent_jobs = []
    for j in recent_result.scalars().all():
        host = await db.get(Host, j.host_id)
        recent_jobs.append({
            "id": j.id,
            "hostname": host.hostname if host else f"host#{j.host_id}",
