    ...
    async with async_session() as db:
        ...
        try:
            ...
            try:
                ...
                async with httpx.AsyncClient(timeout=None) as client:  # No timeout - let it complete
                    pushed = False
                    last_error = None
                    ...
                    if not pushed:
                        raise Exception(f"Upload failed on all candidate ports. Last error: {last_error}")
...
_ALLOWED_CMD_PREFIXES = {
    ...
    "df", "du", "free", "top", "uptime", "date", "uname", "hostname", "whoami", "id",
    # Process / network
    "ps", "netstat", "ss", "ip addr", "ip route", "ip link", "ping",
    # File inspection (read-only)
    "ls", "cat", "tail", "head", "grep", "find", "awk", "sed",
    # Journaling / logs
    "journalctl", "dmesg",
    # Service status (status only)
    "systemctl status", "systemctl is-active", "systemctl is-enabled", "systemctl list-units",
    # Hardware
    "lsblk", "lspci", "lsusb", "lscpu", "lsmem",
    # Package info (read-only list/query)
    "apt list", "apt-cache", "dpkg -l", "dpkg --list", "dpkg -s",
    "dnf list", "yum list", "rpm -q", "rpm -qa", "rpm -qi",
    "pacman -Q", "pacman -Si",
    "pkg info", "pkg list",
    "apk info", "apk list",
    "zypper info", "zypper se",
    # Windows
    "winget list", "winget show",
    "get-service", "get-process", "get-computerinfo",
    "dir", "type", "ipconfig", "netstat",
    # Disk usage
    "lsof",
}
    ...
    async def _write():
        try:
            async with async_session() as _db:
                ...
                await log_action(
                    _db, user, "live_command",
                    target_type="host", target_id=host_ip,
                    details={"command": cmd},
                )
        except Exception:
            pass
...
async def run_command_by_host_id(
    ...
    if not cmd:
        raise HTTPException(400, "Command cannot be empty")
    if not _is_command_allowed(cmd):
        raise HTTPException(400, "Command not permitted. Only pre-approved read-only commands are allowed.")
    if body.timeout < 1 or body.timeout > 300:
        raise HTTPException(400, "Timeout must be 1–300 seconds")
