...
async def _seed_admin_task_templates(db: AsyncSession) -> None:
    existing = (
        await db.execute(select(AdminTaskTemplate).where(AdminTaskTemplate.task_key.in_([row["task_key"] for row in _DEFAULT_ADMIN_TASKS])))
    ).scalars().all()
...
def _policy_execution_public(execution: PolicyExecution) -> dict[str, Any]:
    return {
        ...
        "requested_at": execution.requested_at.isoformat() if execution.requested_at else None,
...
def _policy_public(policy: Policy) -> dict[str, Any]:
    ...
    for revision in revisions:
        if revision.id == getattr(policy, "active_revision_id", None) or revision.status == "active":
            active_revision = revision
            break
    latest_revision = max(revisions, key=lambda row: row.revision_number, default=None)
    last_execution = max(list(getattr(policy, "executions", []) or []), key=lambda row: row.requested_at or datetime.min, default=None)
    return {
        "id": policy.id,
        "name": policy.name,
        "description": policy.description or "",
        "yaml_content": active_revision.yaml_content if active_revision else policy.yaml_content,
        "created_by": policy.created_by or "",
        "created_at": policy.created_at.isoformat() if policy.created_at else None,
        "updated_at": policy.updated_at.isoformat() if policy.updated_at else None,
        "active_revision_id": getattr(policy, "active_revision_id", None),
        "latest_revision_number": int(getattr(policy, "latest_revision_number", 0) or 0),
        "revision_count": len(revisions),
        "active_revision": _policy_revision_public(active_revision) if active_revision else None,
        "latest_revision": _policy_revision_public(latest_revision) if latest_revision else None,
        "last_execution": _policy_execution_public(last_execution) if last_execution else None,
    }
...
async def _get_revision(db: AsyncSession, policy: Policy, revision_id: int | None) -> PolicyRevision | None:
    revisions = list(getattr(policy, "revisions", []) or [])
...
def _render_command_template(command_template: str, parameters: dict[str, Any]) -> str:
    ...
    if "{{" in command or "}}" in command:
        raise HTTPException(status_code=400, detail="Missing template parameters for admin task execution")
    return command


async def _run_command_on_host(db: AsyncSession, host_id: int, command: str, timeout: int, working_dir: str = "") -> dict[str, Any]:
    ...
    if response.status_code >= 400:
        raise HTTPException(status_code=502, detail=payload.get("error") or payload.get("detail") or "Agent command failed")
...
async def apply_policy_on_host(host_ip: str, policy_content: str, host=None) -> dict:
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            resp = await client.post(url, json={"policy": policy_content}, headers=_agent_auth_headers(host))
            if resp.status_code == 200:
                result = resp.json()
                logger.info("Policy applied on %s: %s", host_ip, result)
                return {"host": host_ip, "success": True, "result": result}
            logger.error("Failed to apply policy on %s (HTTP %s): %s", host_ip, resp.status_code, resp.text)
            return {"host": host_ip, "success": False, "error": resp.text, "status_code": resp.status_code}
...
async def apply_policy_to_hosts(
    ...
    *,
        ...
        results = [
            {
                ...
                "result": {
                    "would_apply": True,
                    "policy_revision_id": revision.id if revision else getattr(policy, "active_revision_id", None),
                    "guardrails": dict(guardrails or {}),
                },
            }
            for host in hosts
    ...
    execution = None
    if hasattr(db, "add"):
        execution = await _write_policy_execution(db, policy, revision, mode, host_ids, results, current_user.username, guardrails)
...
async def _execute_admin_task_run(execution_id: int) -> dict[str, Any]:
    async with async_session() as db:
        ...
        query = (
            ...
            .options(selectinload(AdminTaskExecution.template), selectinload(AdminTaskExecution.host))
            .where(AdminTaskExecution.id == execution_id)
        )
        execution = (await db.execute(query)).scalar_one_or_none()
        if not execution:
            raise HTTPException(status_code=404, detail="Admin task execution not found")
        template = execution.template
        if not template:
            raise HTTPException(status_code=404, detail="Admin task template not found")
        command = _render_command_template(template.command_template, dict(execution.parameters or {}))
...
async def list_policies(
    ...
    await _ensure_policy_schema(db)
    query = select(Policy).options(selectinload(Policy.revisions), selectinload(Policy.executions)).order_by(desc(Policy.created_at))
...
async def update_policy(
    ...
    yaml_content = body.yaml_content if body.yaml_content is not None else (active_revision.yaml_content if active_revision else policy.yaml_content)
    ...
    return {"policy": _policy_public(fresh), "draft_revision": _policy_revision_public(revision)}
...
async def list_policy_revisions(
    ...
    await _ensure_policy_schema(db)
    query = select(PolicyRevision).where(PolicyRevision.policy_id == policy_id).order_by(PolicyRevision.revision_number.desc())
    rows = (await db.execute(query)).scalars().all()
...
async def list_policy_executions(
    ...
    await _ensure_policy_schema(db)
    query = select(PolicyExecution).where(PolicyExecution.policy_id == policy_id).order_by(PolicyExecution.requested_at.desc())
    rows = (await db.execute(query)).scalars().all()
...
async def activate_policy_revision(
    ...
    await db.commit()
    fresh = await _get_policy_with_relations(db, policy_id)
    return {"status": "ok", "policy": _policy_public(fresh), "active_revision": _policy_revision_public(revision)}
...
async def list_admin_task_templates(
    ...
    await db.commit()
    rows = (await db.execute(select(AdminTaskTemplate).order_by(AdminTaskTemplate.name.asc()))).scalars().all()
...
async def execute_admin_task(
    ...
    if allowed_roles and current_user.role.value not in allowed_roles:
        raise HTTPException(status_code=403, detail="Current role cannot execute this admin task")
    ...
    queue_job = await enqueue_operation(
        ...
        payload={"template_id": template.id, "task_key": template.task_key, "host_id": host.id},
        runner=runner,
        requested_by=current_user.username,
    )
    ...
    return {"status": "accepted", "execution": _admin_task_execution_public(fresh), "job": queue_job}
