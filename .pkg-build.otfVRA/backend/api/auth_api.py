...
from auth import (
    ...
    verify_and_rotate_refresh_token, revoke_refresh_token, revoke_all_user_tokens,
    get_current_user, require_role, SECRET_KEY, ALGORITHM
)
...
ALL_FEATURES = [
    ...
    'monitoring', 'testing', 'local-repo', 'software', 'policies', 'reports',
    'backups', 'backup_db', 'backup_file', 'backup_vm', 'backup_live',
    'linux_patching', 'windows_patching', 'wsus',
    'cicd_view', 'cicd_manage', 'cicd_execute', 'cicd_approve',
]

ROLE_DEFAULTS = {
    'admin':    {f: True for f in ALL_FEATURES},
    'operator': {f: True for f in ALL_FEATURES if f not in ('audit', 'users', 'license', 'git')},
    'viewer':   {f: True for f in ['dashboard','compliance','hosts','groups','compare','cve','jobs','onboarding','settings']},
    'auditor':  {f: True for f in ['dashboard','compliance','hosts','groups','compare','cve','jobs','audit','onboarding','settings', 'cicd', 'cicd_view']},
}
...
_COMMON_PASSWORDS = {
    "password", "password1", "Password1", "Password1!", "123456", "12345678",
    "qwerty", "letmein", "welcome", "admin", "admin123", "Admin123!",
...
def validate_password_strength(password: str):
    ...
    if not any(not c.isalnum() for c in password):
        raise HTTPException(400, "Password must contain at least one special character (!@#$%^&* etc).")
    if password.lower() in _COMMON_PASSWORDS or password in _COMMON_PASSWORDS:
        raise HTTPException(400, "Password is too common. Please choose a more unique password.")
...
async def login(
    ...
    response.set_cookie(
        key=cookie_name, value=access_token,
        max_age=access_max_age, expires=access_max_age,
        path="/", domain=cookie_domain, secure=cookie_secure,
        httponly=True, samesite=cookie_samesite if cookie_samesite in ("lax", "strict", "none") else "lax",
    )
    ...
    response.set_cookie(
        ...
        max_age=refresh_max_age, expires=refresh_max_age,
        path="/api/auth/refresh", domain=cookie_domain, secure=cookie_secure,
...
async def refresh_token(
    ...
    raw_refresh = request.cookies.get(REFRESH_COOKIE) or (request.headers.get("X-Refresh-Token", ""))
    if not raw_refresh:
        raise HTTPException(status.HTTP_401_UNAUTHORIZED, "No refresh token provided")

    client_ip = request.client.host if request.client else ""
    ua = request.headers.get("User-Agent", "")[:512]
    user = await verify_and_rotate_refresh_token(raw_refresh, db, ip_address=client_ip, user_agent=ua)

    # Issue new tokens
    access_token = create_access_token({"sub": user.username, "role": user.role.value})
    new_raw_refresh = await create_refresh_token(user.id, db, ip_address=client_ip, user_agent=ua)
    await db.commit()
    ...
    response.set_cookie(key=REFRESH_COOKIE, value=new_raw_refresh,
        max_age=refresh_max_age, path="/api/auth/refresh", domain=cookie_domain,
        secure=cookie_secure, httponly=True, samesite="strict")

    perms = get_effective_permissions(user.role.value, user.custom_permissions)
    return {"access_token": access_token, "token_type": "bearer",
            "user": {"id": user.id, "username": user.username, "role": user.role.value, "permissions": perms}}
...
async def forgot_password(req: ForgotPasswordRequest, db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(select(User).where((User.username == req.username_or_email) | (User.email == req.username_or_email)))
    user = result.scalar_one_or_none()
    if not user:
    ...
    token = jwt.encode(
        {"sub": user.username, "purpose": "reset", "exp": datetime.now(timezone.utc) + timedelta(hours=1)},
        SECRET_KEY,
        algorithm=ALGORITHM,
    )
...
async def register_user(req: RegisterRequest, db: AsyncSession = Depends(get_db)):
    ...
    if count > 0:
        ...
        raise HTTPException(403, "Public registration is only available for the first administrator. Please contact your system admin.")
