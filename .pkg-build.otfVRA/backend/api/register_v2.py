...
def _detect_os_family(os_name: str) -> str:
    ...
    if "windows" in s:
        return "windows"
    if any(x in s for x in ["ubuntu", "debian", "linux mint", "pop!_os", "pop os", "kali"]):
        return "debian"
    if any(x in s for x in ["red hat", "rhel", "centos", "rocky", "alma", "almalinux", "fedora", "oracle linux"]):
        return "rhel"
    return "other"


async def _get_or_create_group(db: AsyncSession, name: str, description: str) -> HostGroup:
...
async def _ensure_host_schema(db: AsyncSession) -> None:
    ...
    for stmt in stmts:
        try:
            ...
            pass
    try:
        await db.execute(text("CREATE INDEX IF NOT EXISTS ix_hosts_agent_id ON hosts(agent_id);"))
    except Exception:
        pass
    try:
        await db.execute(text("CREATE INDEX IF NOT EXISTS ix_hosts_site ON hosts(site);"))
    except Exception:
        pass
...
async def _ensure_os_family_group(db: AsyncSession, host: Host, os_name: str) -> None:
    ...
    result = await db.execute(
        select(HostGroup).join(
            host_group_assoc, HostGroup.id == host_group_assoc.c.group_id
...
async def _find_host_by_agent_id(db: AsyncSession, agent_id: str):
    ...
    logging.getLogger("patchmaster.register").warning(
        "Duplicate agent_id '%s' found in %d rows — using most recent", agent_id, len(rows)
    )
    rows.sort(key=lambda x: x.updated_at or x.created_at or _utcnow_naive(), reverse=True)
    return rows[0]


async def _find_legacy_host(db: AsyncSession, hostname: str, ip: str, incoming_agent_id: str = ""):
    """
    ...
    if ip:
        ...
        if hosts:
            ...
            if not hosts:
                return None
            exact_name = [h for h in hosts if (h.hostname or "").strip().lower() == clean_name and clean_name]
            if exact_name:
                exact_name.sort(key=lambda x: x.updated_at or x.created_at or _utcnow_naive(), reverse=True)
                return exact_name[0]
            hosts.sort(key=lambda x: x.updated_at or x.created_at or _utcnow_naive(), reverse=True)
            return hosts[0]
    if hostname:
        result = await db.execute(
            select(Host).options(selectinload(Host.groups)).where(Host.hostname == hostname)
        )
...
async def _allocate_hostname(db: AsyncSession, base_hostname: str, os_name: str) -> str:
    ...
    family = _detect_os_family(os_name)
    suffix = {"windows": "windows", "debian": "linux", "rhel": "linux", "other": "agent"}[family]
...
async def register_agent(req: RegisterRequest, request: Request, db: AsyncSession = Depends(get_db)):
    try:
        ...
        if not host:
            host = await _find_legacy_host(db, req.hostname, clean_ip, incoming_agent_id=clean_agent_id)

        if host:
            if clean_agent_id:
                host.agent_id = clean_agent_id
            if req.hostname and not host.hostname:
        ...
        try:
            ...
            _log.getLogger("patchmaster.register").warning("Prometheus target sync failed (register): %s", exc)
        await db.commit()
...
async def heartbeat(request: Request, db: AsyncSession = Depends(get_db)):
    try:
        ...
        hardware_inventory = _clean_hardware_inventory(body.get("hardware_inventory", {}))
        ...
        try:
            ...
            import logging as _log
            _log.getLogger("patchmaster.register").warning("Prometheus target sync failed (heartbeat): %s", exc)
