...
class AlertAction(Base):
    ...
    ticket_id = Column(Integer, ForeignKey("alert_tickets.id", ondelete="SET NULL"), nullable=True, index=True)
...
class Host(Base):
    ...
    __table_args__ = (
        ...
        # Prevents duplicate agent_id rows while allowing multiple rows with agent_id=''.
        Index(
...
class RunbookSchedule(Base):
    ...
    id = Column(Integer, primary_key=True)
    profile_id = Column(Integer, ForeignKey("runbook_profiles.id", ondelete="CASCADE"), nullable=False, index=True)
    name = Column(String(150), nullable=False)
...
class RunbookExecution(Base):
    ...
    id = Column(Integer, primary_key=True)
    profile_id = Column(Integer, ForeignKey("runbook_profiles.id", ondelete="SET NULL"), nullable=True, index=True)
    schedule_id = Column(Integer, ForeignKey("runbook_schedules.id", ondelete="SET NULL"), nullable=True, index=True)
...
class HostCVE(Base):
    ...
    cve_id = Column(String(50), ForeignKey("cves.id", ondelete="CASCADE"), primary_key=True)
...
class BackupLog(Base):
    ...
    id = Column(Integer, primary_key=True)
    config_id = Column(Integer, ForeignKey("backup_configs.id", ondelete="CASCADE"), nullable=False)
    status = Column(Enum(JobStatus), default=JobStatus.pending)
...
class RestoreDrillRun(Base):
    ...
    id = Column(Integer, primary_key=True)
    config_id = Column(Integer, ForeignKey("backup_configs.id", ondelete="CASCADE"), nullable=False, index=True)
    host_id = Column(Integer, ForeignKey("hosts.id", ondelete="CASCADE"), nullable=False, index=True)
    backup_log_id = Column(Integer, ForeignKey("backup_logs.id", ondelete="SET NULL"), nullable=True, index=True)
...
class PolicyRevision(Base):
    ...
    status = Column(String(24), default="draft", index=True)  # draft | active | archived
...
class PolicyExecution(Base):
    ...
    id = Column(Integer, primary_key=True)
    policy_id = Column(Integer, ForeignKey("policies.id", ondelete="CASCADE"), nullable=False, index=True)
    revision_id = Column(Integer, ForeignKey("policy_revisions.id", ondelete="SET NULL"), nullable=True, index=True)
    execution_mode = Column(String(24), default="apply", index=True)  # dry_run | apply | rollback
...
class AdminTaskExecution(Base):
    ...
    id = Column(Integer, primary_key=True)
    template_id = Column(Integer, ForeignKey("admin_task_templates.id", ondelete="SET NULL"), nullable=True, index=True)
...
class RingRolloutPolicy(Base):
    ...
    runs = relationship("RingRolloutRun", back_populates="policy", cascade="all, delete-orphan")


class RingRolloutRun(Base):
    __tablename__ = "ring_rollout_runs"
    id = Column(Integer, primary_key=True)
    policy_id = Column(Integer, ForeignKey("ring_rollout_policies.id", ondelete="CASCADE"), nullable=False, index=True)
    status = Column(Enum(RingRolloutStatus), default=RingRolloutStatus.pending, index=True)
...
class CICDBuild(Base):
    ...
    id = Column(Integer, primary_key=True)
    pipeline_id = Column(Integer, ForeignKey("cicd_pipelines.id", ondelete="CASCADE"), nullable=False, index=True)
...
class CICDDeployment(Base):
    ...
    id = Column(Integer, primary_key=True)
    pipeline_id = Column(Integer, ForeignKey("cicd_pipelines.id", ondelete="CASCADE"), nullable=False, index=True)
    environment_id = Column(Integer, ForeignKey("cicd_environments.id", ondelete="SET NULL"), nullable=True, index=True)
    build_id = Column(Integer, ForeignKey("cicd_builds.id", ondelete="SET NULL"), nullable=True, index=True)
    ...
    __table_args__ = (
        Index("ix_cicd_deployments_pipeline_id_created_at", "pipeline_id", "created_at"),
    )
...
class CICDDeploymentApproval(Base):
    ...
    id = Column(Integer, primary_key=True)
    deployment_id = Column(Integer, ForeignKey("cicd_deployments.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    __table_args__ = (
        UniqueConstraint("deployment_id", "approver", name="uq_cicd_deployment_approval_deployment_user"),
        Index("ix_cicd_deployment_approval_deployment_decision", "deployment_id", "decision"),
    )
...
class CICDDeploymentApprovalEvent(Base):
    ...
    __table_args__ = (
        Index("ix_cicd_deployment_approval_events_deployment_created", "deployment_id", "created_at"),
    )
...
class CICDNotificationDeliveryReceipt(Base):
    ...
    __table_args__ = (
        Index("ix_cicd_delivery_receipt_deployment_event", "deployment_id", "event_type"),
        Index("ix_cicd_delivery_receipt_status_next_retry", "status", "next_retry_at"),
        UniqueConstraint("deployment_id", "event_type", "channel_type", "target", name="uq_cicd_delivery_receipt_key"),
    )
...
class CICDBuildStageRun(Base):
    ...
    id = Column(Integer, primary_key=True)
    build_id = Column(Integer, ForeignKey("cicd_builds.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    __table_args__ = (
        ...
        UniqueConstraint("build_id", "stage_name", name="uq_cicd_stage_runs_build_stage"),
    )
...
class GitRepository(Base):
    ...
    __table_args__ = (
        UniqueConstraint("provider", "repo_full_name", name="uq_git_repo_provider_fullname"),
...
class SLAViolation(Base):
    ...
    id = Column(Integer, primary_key=True)
    host_id = Column(Integer, ForeignKey("hosts.id", ondelete="CASCADE"), nullable=False)
    cve_id = Column(String(50), ForeignKey("cves.id", ondelete="CASCADE"), nullable=False)
    sla_id = Column(Integer, ForeignKey("patch_slas.id", ondelete="SET NULL"), nullable=True)
    detected_at = Column(DateTime, default=_utcnow)
...
class CVERemediation(Base):
    ...
    assignee = relationship("User", backref="assigned_remediations", foreign_keys=[assigned_to])
...
class HookExecution(Base):
    ...
    id = Column(Integer, primary_key=True)
    hook_id = Column(Integer, ForeignKey("patch_hooks.id", ondelete="CASCADE"), nullable=False)
    job_id = Column(Integer, ForeignKey("patch_jobs.id", ondelete="CASCADE"), nullable=True)
    host_id = Column(Integer, ForeignKey("hosts.id", ondelete="CASCADE"), nullable=False)
    trigger = Column(Enum(HookTrigger), nullable=False)
...
class BulkPatchJob(Base):
    ...
    auto_snapshot = Column(Boolean, default=True)
    auto_rollback = Column(Boolean, default=True)
    status = Column(String(20), default="pending")  # pending | running | jobs_created | failed
    ...
    # Bug #14 fix: prevent duplicate bulk job names within the same second
    __table_args__ = (
        UniqueConstraint("name", "created_at", name="uq_bulk_patch_job_name_created_at"),
...
class MirrorRepo(Base):
    ...
    provider = Column(Enum(MirrorRepoProvider), default=MirrorRepoProvider.custom, nullable=False)
    ...
    sync_runs = relationship("MirrorSyncRun", back_populates="repo", cascade="all, delete-orphan")
    package_index = relationship("MirrorPackageIndex", back_populates="repo", cascade="all, delete-orphan")
    sync_lease = relationship("MirrorSyncLease", back_populates="repo", cascade="all, delete-orphan", uselist=False, passive_deletes=True)
...
class MirrorSyncRun(Base):
    ...
    id = Column(Integer, primary_key=True)
    repo_id = Column(Integer, ForeignKey("mirror_repos.id", ondelete="CASCADE"), nullable=False, index=True)
...
class MirrorSyncLease(Base):
    ...
    id = Column(Integer, primary_key=True)
    repo_id = Column(Integer, ForeignKey("mirror_repos.id", ondelete="CASCADE"), nullable=False, unique=True, index=True)
...
class MirrorPackageIndex(Base):
    ...
    __table_args__ = (
        UniqueConstraint("repo_id", "package_name", "package_version", "architecture", name="uq_mirror_pkg_repo_name_ver_arch"),
...
class PluginIntegration(Base):
    ...
    id = Column(Integer, primary_key=True)
    name = Column(String(150), nullable=False, unique=True, index=True)
    plugin_type = Column(Enum(PluginType), nullable=False, default=PluginType.webhook, index=True)
    is_enabled = Column(Boolean, default=True, index=True)
    config = Column(JSON, default=dict)
    ...
    delivery_logs = relationship("PluginDeliveryLog", back_populates="plugin", cascade="all, delete-orphan")


class PluginDeliveryLog(Base):
    __tablename__ = "plugin_delivery_logs"
    id = Column(Integer, primary_key=True)
    plugin_id = Column(Integer, ForeignKey("plugin_integrations.id", ondelete="CASCADE"), nullable=False, index=True)
    event_type = Column(String(120), nullable=False, index=True)
    status = Column(Enum(PluginDeliveryStatus), default=PluginDeliveryStatus.pending, index=True)
    request_payload = Column(JSON, default=dict)
...
class HostTimelineEvent(Base):
    ...
    id = Column(Integer, primary_key=True)
    host_id = Column(Integer, ForeignKey("hosts.id", ondelete="CASCADE"), nullable=False, index=True)
    event_type = Column(String(50), nullable=False)  # patch | snapshot | cve | reboot | agent_update | login
    title = Column(String(200), nullable=False)
...
class CICDSecret(Base):
    ...
    pipeline_id = Column(Integer, ForeignKey("cicd_pipelines.id", ondelete="CASCADE"), nullable=True, index=True)
...
class SoftwareKioskRequest(Base):
    ...
    id = Column(Integer, primary_key=True)
    catalog_item_id = Column(Integer, ForeignKey("software_catalog_items.id", ondelete="CASCADE"), nullable=False, index=True)
    host_id = Column(Integer, ForeignKey("hosts.id", ondelete="CASCADE"), nullable=False, index=True)
...
class ProvisioningTemplate(Base):
    ...
    name = Column(String(160), nullable=False, unique=True, index=True)
    description = Column(Text, default="")
    source_host_id = Column(Integer, ForeignKey("hosts.id", ondelete="SET NULL"), nullable=True, index=True)
    ...
    runs = relationship("ProvisioningRun", back_populates="template", cascade="all, delete-orphan")


class ProvisioningRun(Base):
    __tablename__ = "provisioning_runs"
    id = Column(Integer, primary_key=True)
    template_id = Column(Integer, ForeignKey("provisioning_templates.id", ondelete="CASCADE"), nullable=False, index=True)
    initiated_by = Column(String(100), default="")
...
class BootRelay(Base):
    ...
    id = Column(Integer, primary_key=True)
    host_id = Column(Integer, ForeignKey("hosts.id", ondelete="CASCADE"), nullable=False, unique=True, index=True)
    name = Column(String(160), nullable=False, unique=True, index=True)
    site_scope = Column(String(120), default="", index=True)
    ...
    runs = relationship("BootRelayRun", back_populates="relay", cascade="all, delete-orphan")
...
class NetworkBootNetwork(Base):
    ...
    relay_id = Column(Integer, ForeignKey("boot_relays.id", ondelete="SET NULL"), nullable=True, index=True)
...
class NetworkBootProfile(Base):
    ...
    id = Column(Integer, primary_key=True)
    name = Column(String(160), nullable=False, unique=True, index=True)
    network_id = Column(Integer, ForeignKey("network_boot_networks.id", ondelete="SET NULL"), nullable=True, index=True)
    provisioning_template_id = Column(Integer, ForeignKey("provisioning_templates.id", ondelete="SET NULL"), nullable=True, index=True)
    mirror_repo_id = Column(Integer, ForeignKey("mirror_repos.id", ondelete="SET NULL"), nullable=True, index=True)
    os_family = Column(String(40), default="", index=True)
...
class NetworkBootAssignment(Base):
    ...
    id = Column(Integer, primary_key=True)
    host_id = Column(Integer, ForeignKey("hosts.id", ondelete="SET NULL"), nullable=True, index=True)
    network_id = Column(Integer, ForeignKey("network_boot_networks.id", ondelete="CASCADE"), nullable=False, index=True)
    profile_id = Column(Integer, ForeignKey("network_boot_profiles.id", ondelete="CASCADE"), nullable=False, index=True)
...
class BootRelayRun(Base):
    ...
    id = Column(Integer, primary_key=True)
    relay_id = Column(Integer, ForeignKey("boot_relays.id", ondelete="CASCADE"), nullable=False, index=True)
...
class BootSession(Base):
    ...
    assignment_id = Column(Integer, ForeignKey("network_boot_assignments.id", ondelete="SET NULL"), nullable=True, index=True)
    profile_id = Column(Integer, ForeignKey("network_boot_profiles.id", ondelete="SET NULL"), nullable=True, index=True)
    network_id = Column(Integer, ForeignKey("network_boot_networks.id", ondelete="SET NULL"), nullable=True, index=True)
    relay_id = Column(Integer, ForeignKey("boot_relays.id", ondelete="SET NULL"), nullable=True, index=True)
    host_id = Column(Integer, ForeignKey("hosts.id", ondelete="SET NULL"), nullable=True, index=True)
    ...
    host = relationship("Host")
    events = relationship("BootEvent", back_populates="session", cascade="all, delete-orphan")


class BootEvent(Base):
    __tablename__ = "boot_events"
    id = Column(Integer, primary_key=True)
    session_id = Column(Integer, ForeignKey("boot_sessions.id", ondelete="CASCADE"), nullable=False, index=True)
...
class CICDAgentRun(Base):
    ...
    id = Column(Integer, primary_key=True)
    build_id = Column(Integer, ForeignKey("cicd_builds.id", ondelete="CASCADE"), nullable=False, index=True)
    target_id = Column(Integer, ForeignKey("cicd_agent_targets.id", ondelete="SET NULL"), nullable=True, index=True)
    host_id = Column(Integer, ForeignKey("hosts.id", ondelete="SET NULL"), nullable=True, index=True)
