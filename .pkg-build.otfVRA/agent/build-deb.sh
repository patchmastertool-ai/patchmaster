...
if [[ -d "$WORKSPACE_WHEELS" ]]; then
  ...
  PIP_OPTS="--find-links ${WORKSPACE_WHEELS}"
elif [[ -d "$DEFAULT_WHEEL_DIR" ]]; then
  echo "    Using wheels from vendor/wheels"
  PIP_OPTS="--find-links ${DEFAULT_WHEEL_DIR}"
elif [[ -d "$CACHE_WHEELS" ]]; then
  echo "    Using wheels from cache"
  PIP_OPTS="--find-links ${CACHE_WHEELS}"
else
  echo "    No local wheels found, will download from PyPI"
fi

export PIP_DISABLE_PIP_VERSION_CHECK=1
VENV_PIP="${BUILD_ROOT}${INSTALL_DIR}/venv/bin/pip"
$VENV_PIP install $PIP_OPTS -q setuptools wheel || $VENV_PIP install -q setuptools wheel
$VENV_PIP install $PIP_OPTS -q -r "${SCRIPT_DIR}/requirements.txt" || $VENV_PIP install -q -r "${SCRIPT_DIR}/requirements.txt"
