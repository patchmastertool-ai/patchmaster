...
def _recent_log_snippet(log_dir: Path, max_files: int = 3, tail_lines: int = 40) -> str:
    try:
        files = sorted(glob.glob(str(log_dir / "*.log")), key=lambda p: os.path.getmtime(p), reverse=True)[:max_files]
        out = []
        for fp in files:
            try:
...
def _write_service_xml(
    ...
    work_line = ""
    if working_dir:
        work_line = f"  <workingdirectory>{escape(str(working_dir))}</workingdirectory>\n"
...
def _install_services(
    ...
    log_dir = Path(os.environ.get("ProgramData", r"C:\ProgramData")) / "PatchMaster-Agent" / "logs"
    ...
    _write_service_xml(
        ...
        env={
            ...
            **({"PATCHMASTER_SITE": site.strip()[:120]} if site and site.strip() else {}),
        },
        log_dir=log_dir,
        working_dir=install_dir,
    )
    ...
    _run([str(api_wrapper), "start"])
    _run([str(hb_wrapper), "start"])
    ...
    if master_host:
        controller_ok = _check_tcp_connectivity(master_host, int(master_port), timeout=4.0)
        print(f"[CHECK] controller {master_host}:{master_port} reachable={controller_ok}")
        if require_connectivity and not controller_ok:
            raise RuntimeError(f"Controller {master_host}:{master_port} is not reachable from this host")
    ...
    if not api_ok:
        if api_port_open and not strict_health:
            print(f"[WARN] API health endpoint did not return 2xx yet, but port {agent_port} is open. Continuing install (non-strict mode).")
        else:
            snippet = _recent_log_snippet(log_dir)
            extra = f"\nRecent logs:{snippet}" if snippet else ""
            raise RuntimeError(f"API health check failed on port {agent_port}. Check service logs in ProgramData\\PatchMaster-Agent\\logs.{extra}")


def _uninstall_services(install_dir: Path, agent_port: str = "8080", metrics_port: str = "9100"):
    service_api = "PatchMaster-Agent"
    service_hb = "PatchMaster-Agent-Heartbeat"
    api_wrapper = install_dir / f"{service_api}.exe"
    hb_wrapper = install_dir / f"{service_hb}.exe"
...
def _cleanup_files(install_dir: Path, agent_port: str = "8080", metrics_port: str = "9100"):
    program_data = Path(os.environ.get("ProgramData", r"C:\ProgramData")) / "PatchMaster-Agent"
    _kill_processes()
...
def main():
    ...
    p.add_argument("--master-url", default=None, help="PatchMaster master URL (e.g. http://192.168.1.10:8000)")
    ...
    p.add_argument("--site", default=os.environ.get("PATCHMASTER_SITE") or os.environ.get("PATCHMASTER_LOCATION") or "", help="Optional site or location label for this endpoint")
    p.add_argument("--allow-from", default=os.environ.get("PM_AGENT_ALLOW_FROM") or None, help="Optional remote IP/CIDR allowlist for Windows Firewall rules")
    p.add_argument("--wbadmin-target", default=os.environ.get("PM_WINDOWS_WBADMIN_TARGET") or os.environ.get("WBADMIN_BACKUP_TARGET") or None, help="Windows wbadmin backup target (e.g. \\\\server\\share or E:)")
    p.add_argument("--wbadmin-share-name", default=os.environ.get("PM_WINDOWS_WBADMIN_SHARE_NAME") or "patchmaster-wbadmin")
    p.add_argument("--require-connectivity", action="store_true", help="Fail install if controller connectivity check does not pass")
    p.add_argument("--strict-health", action="store_true", help="Fail install when /health is not 2xx even if API port is open")
    p.add_argument("--install-dir", default=str(Path(os.environ.get("ProgramFiles", r"C:\Program Files")) / "PatchMaster-Agent"))
    p.add_argument("--no-pause", action="store_true")
    p.add_argument("--uninstall", action="store_true", help="Stop services and remove PatchMaster Agent files")
    args = p.parse_args()
    ...
    preferred_windows_port = (os.environ.get("PM_WINDOWS_AGENT_PORT") or "18080").strip()
    agent_port = (args.agent_port or existing_agent_port or preferred_windows_port).strip()
    if (not args.agent_port) and existing_agent_port == "8080" and preferred_windows_port != "8080":
    ...
    master_url = args.master_url or os.environ.get("MASTER_URL") or _read_existing_master_url(install_dir)
    if not master_url:
        if args.no_pause:
            print("[ERROR] MASTER_URL is required (use --master-url).")
            return 1
        master_url = input("Enter PatchMaster Master URL (example: http://192.168.1.10:8000): ").strip()
    if not master_url:
    ...
    if not wbadmin_target:
        try:
            parsed = urlsplit(master_url)
            share_name = (args.wbadmin_share_name or "patchmaster-wbadmin").strip().strip("\\/")
    ...
    if not src_api.exists() or not src_hb.exists() or not src_winsw.exists():
        print("[ERROR] Installer payload is missing (patch-agent.exe / patch-agent-heartbeat.exe / winsw.exe).")
        if not args.no_pause:
            input("Press Enter to exit...")
        return 1
