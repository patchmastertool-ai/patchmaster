...
class PacmanManager(BasePackageManager):
            ...
            if len(parts) >= 2:
                ...
                if self.aur_helper:
                    ...
                    if rc_check == 0:
                        ...
                        rc_repo, repo_out = run_cmd(["pacman", "-Qi", pkg_name], timeout=5)
                        if rc_repo == 0 and "Repository" in repo_out:
                            ...
                            for repo_line in repo_out.split("\n"):
                                if repo_line.startswith("Repository"):
                                    if "None" in repo_line or "local" in repo_line.lower():
                                        is_aur = True
                                    break
    ...
    def install_aur_helper(self):
        ...
        if rc != 0:
            return 1, "git is required but not installed. Install with: pacman -S git base-devel"
        ...
        return 1, "AUR helper installation requires manual setup. See: https://github.com/Jguer/yay#installation"
...
class ApkManager(BasePackageManager):
    ...
    def list_upgradable(self):
        ...
        for line in out.strip().split("\n"):
            ...
            if len(parts) >= 3 and parts[1] == "<":
                ...
                if len(pkg_parts) >= 2:
                    pkg_name = "-".join(pkg_parts[:-2]) if len(pkg_parts) > 2 else pkg_parts[0]
                    current_ver = "-".join(pkg_parts[-2:])
                    
                    avail_parts = available.rsplit("-", 2)
                    avail_ver = "-".join(avail_parts[-2:]) if len(avail_parts) >= 2 else available
    ...
    def install(self, packages, local=False, security_only=False, 
        ...
        # Note: Alpine doesn't have security-only updates concept
        # All updates are treated equally
...
class FreeBSDPkgManager(BasePackageManager):
    ...
    def install(self, packages, local=False, security_only=False, 
        ...
        # Note: FreeBSD doesn't have security-only updates concept
...
def get_pkg_manager():
    ...
    if os.path.exists("/usr/bin/pacman"):
        ...
        if os.path.exists("/etc/os-release"):
            try:
                with open("/etc/os-release") as f:
                    content = f.read().lower()
                    if "id=arch" in content or "id_like=arch" in content or "id=manjaro" in content:
                        return PacmanManager()
            except:
                pass
    ...
    if os.path.exists("/usr/bin/dnf"): return DnfManager()  # Fedora, RHEL 8+, CentOS 8+
    ...
    try:
        ...
        if not IS_WINDOWS:
            ...
            if isinstance(pkg_mgr, PacmanManager):
                ...
                if rc == 0:
                    with open(os.path.join(snap_dir, "explicit_packages.txt"), "w") as f:
                        f.write(explicit_pkgs)
                    result["details"]["explicit_packages_count"] = len(explicit_pkgs.strip().split("\n"))
            ...
            if isinstance(pkg_mgr, FreeBSDPkgManager):
                ...
                if os.path.isdir("/usr/local/etc/pkg/repos"):
                    for rf in os.listdir("/usr/local/etc/pkg/repos"):
                        shutil.copy2(os.path.join("/usr/local/etc/pkg/repos", rf), sources_dir)
